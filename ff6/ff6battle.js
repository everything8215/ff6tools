...
FF6Battle.prototype.monsterInSlot = function(slot) {
    ...
    var y = this.battleProperties["monster" + slot + "Y"].value + 1;
...
//            map = this.rom.data.subarray(mapBegin, mapEnd);
    ...
    var tiles = new Uint32Array(size * size);
...
FF6Battle.prototype.drawGhostTrain = function(slot) {
    ...
    var g1 = properties.graphics1;
    var g2 = properties.graphics2.value;
    var g3 = properties.graphics3.value;

    // load graphics
    var gfx = new Uint8Array(0x10000);
    if (g1.getSpecialValue() !== 0xFF) gfx.set(this.loadBattleBackgroundGraphics(g1.g.value, g1.double.value), this.rom.isSFC ? 0x0000 : 0x4000);
    if (g3 !== 0xFF) gfx.set(this.loadBattleBackgroundGraphics(g3, false), this.rom.isSFC ? 0x6000 : 0x2000);
    ...
    for (var i = 0; i < layout.length; i++) layout[i] &= 0xFFFF01FF;

    // load palette
    var pal = new Uint32Array(0x80);
    pal.set(gfxProperties.palette.target.data, 0x60);
...
FF6Battle.prototype.drawBackground = function() {
    ...
    var gfx = new Uint8Array(0x10000);
    if (g1.getSpecialValue() !== 0xFF) gfx.set(this.loadBattleBackgroundGraphics(g1.g.value, g1.double.value), this.rom.isSFC ? 0x0000 : 0x4000);
    if (g2 !== 0xFF) gfx.set(this.loadBattleBackgroundGraphics(g2, false), this.rom.isSFC ? 0x2000 : 0x6000);
    if (g3 !== 0xFF) gfx.set(this.loadBattleBackgroundGraphics(g3, false), this.rom.isSFC ? 0xE000 : 0x2000);
    ...
    this.ppu.layers[1].tiles = layout;
...
FF6Battle.prototype.loadBattleBackgroundGraphics = function(i, double) {
    if (i === 0xFF) return new Uint8Array(0); // no graphics

    var bgGraphics = this.rom.battleBackgroundGraphics;
    var pointer = bgGraphics.createPointer(i);

    var begin;
    if (this.rom.isSFC) {
        // normal battle bg graphics
        if (bgGraphics.range.contains(this.rom.mapAddress(pointer.value))) return bgGraphics.item(i).data;

        // use map graphics (absolute pointer)
        begin = this.rom.mapAddress(pointer.value);
    } else {
        // normal battle bg graphics
        if ((pointer.value & 0x800000) === 0) return bgGraphics.item(i).data;

        // use map graphics (absolute pointer)
        pointer.options.offset = this.rom.mapAddress(Number(this.rom.mapGraphics.pointerOffset));
        begin = pointer.value & 0x7FFFFF;
    }
    var end = begin + (double ? 0x2000 : 0x1000);
    var decode = this.rom.isSFC ? GFX.graphicsFormat.snes4bpp.decode : GFX.graphicsFormat.linear4bpp.decode;
