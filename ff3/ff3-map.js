...
class FF3Map extends ROMEditor_ {
    constructor(rom) {
        super(rom);

        this.name = 'FF3Map';
        this.tileset = new FF3MapTileset(rom, this);
        ...
        this.isWorld = false;
        this.tileMask = FF3Map.TileMask[0];
    ...
    resetControls() {
        ...
        // const maskArray = this.isWorld ? FF3Map.WorldTileMask : FF3Map.TileMask;
        // const maskNames = [];
    ...
    selectTileProperties(t) {
        ...
        //     const battleEditor = propertyList.getEditor('FF3Battle');
    ...
    loadMap() {
        ...
        const tileset = this.rom.mapTileset.item(t);
        const nameTable = this.rom.mapNameTable.item(t);

        // get the palette
        const basePaletteObject = this.rom.mapPaletteBase;
        const bgPaletteObject = this.rom.mapPaletteBG;
        const palette = new Uint32Array(32);
        palette.set(basePaletteObject.data.subarray(0, 16));

        // these are bg palettes
        const p0 = this.mapProperties.palette0;
        const p1 = this.mapProperties.palette1;
        const p2 = this.mapProperties.palette2;

        // these are sprite palettes
        const p6 = this.mapProperties.palette6;
        const p7 = this.mapProperties.palette7;

        // background color is always black (see 3A/9142)
        palette[0] = 0xFF000000;

        palette[1] = bgPaletteObject.data[0x0000 + p0.value];
        palette[2] = bgPaletteObject.data[0x0100 + p0.value];
        palette[3] = bgPaletteObject.data[0x0200 + p0.value];

        palette[5] = bgPaletteObject.data[0x0000 + p1.value];
        palette[6] = bgPaletteObject.data[0x0100 + p1.value];
        palette[7] = bgPaletteObject.data[0x0200 + p1.value];

        palette[9] = bgPaletteObject.data[0x0000 + p2.value];
        palette[10] = bgPaletteObject.data[0x0100 + p2.value];
        palette[11] = bgPaletteObject.data[0x0200 + p2.value];

        palette[25] = bgPaletteObject.data[0x0000 + p6.value];
        palette[26] = bgPaletteObject.data[0x0100 + p6.value];
        palette[27] = bgPaletteObject.data[0x0200 + p6.value];

        palette[29] = bgPaletteObject.data[0x0000 + p7.value];
        palette[30] = bgPaletteObject.data[0x0100 + p7.value];
        palette[31] = bgPaletteObject.data[0x0200 + p7.value];

        // load graphics
        const graphics = new Uint8Array(0x2000);
        const graphicsID = this.rom.mapGraphicsID.item(this.m).graphicsSet;
        const graphicsSet = this.rom.mapGraphicsSet.item(graphicsID.value);

        // from 3E/DF00
        const graphicsSetSize = [0x1A,0x08,0x08,0x0E,0x08,0x10,0x10,0x10]

        let graphicsOffset = 0;
        for (let s = 0; s < 8; s++) {
            const pointer = graphicsSet[`graphics${s + 1}`].value;
            const size = graphicsSetSize[s];
            const start = pointer * 4;
            const end = start + size * 0x40;
            const tiles = this.rom.mapGraphics.data.subarray(start, end);
            graphics.set(tiles, graphicsOffset);
            graphicsOffset += size * 0x40;
        }

        // load the tile layout
        const tilemapID = this.rom.mapTilemap.item(this.m).tilemap;
        const layout = this.rom.mapLayout.item(tilemapID.value);
        this.layer[0].loadLayout({
            type: FF3MapLayer.Type.layer1,
            layout: layout,
            tileset: tileset.data,
            paletteAssignment: nameTable.data,
            w: 32,
            h: 32
        });
        ...
        this.observer.startObserving([
            ...
            tileset, nameTable, basePaletteObject, bgPaletteObject,
            graphicsID,
            p0, p1, p2, p6, p7, tilemapID
        ], this.loadMap);
    ...
    loadTriggers() {
        ...
        // load npcs
        const mapNPC = this.rom.npcProperties.item(this.mapProperties.npc.value);
