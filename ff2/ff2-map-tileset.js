...
class FF2MapTileset extends ROMToolbox {
    constructor(rom, map) {
    ...
    mouseDown(e) {
        const xClip = this.map.isWorld ? 16 : 8;
        const tileSize = (this.map.isWorld || this.map.l === 3) ? 16 : 32;
        let x = Math.floor(e.offsetX / this.zoom / tileSize);
        x = Math.min(Math.max(x, 0), xClip - 1);
        let y = Math.floor(e.offsetY / this.zoom / tileSize);
        y = Math.min(Math.max(y, 0), 15);
        this.clickPoint = { x: x, y: y };
        this.mouseMove(e);

        if (this.map.l === 0) this.map.selectTileProperties(x + y * xClip);
    }
    ...
    mouseMove(e) {
        ...
        for (let r = 0; r < h; r++) {
            for (let c = 0; c < w; c++) {
                this.selection.tilemap[c + r * w] = x + c + (y + r) * xClip;
            }
        }
    ...
    loadMap() {
        ...
        if (this.map.isWorld) {
            ...
            this.layer.loadLayout({
                ...
                layout: layout,
                tileset16: this.map.layer[0].tileset16,
                w: 16, h: 8,
            ...
            this.ppu.height = 512;
            this.ppu.layers[0].rows = 64;
            this.tilesetCanvas.height = 512;
            this.layer.loadLayout({
                type: FF2MapLayer.Type.layer1,
                layout: layout,
                tileset16: this.map.layer[0].tileset16,
                tileset32: this.map.layer[0].tileset32,
                w: 8, h: 16,
                paletteAssignment: this.map.layer[0].paletteAssignment
            });
        }
    ...
    drawCursor() {
        ...
        const tileSize = (this.map.isWorld || this.map.l === 3) ? 16 : 32;
        const l = Math.floor(this.selection.x * tileSize * this.zoom);
        const t = Math.floor(this.selection.y * tileSize * this.zoom);
        const r = Math.ceil((this.selection.x + this.selection.w) * tileSize * this.zoom);
        const b = Math.ceil((this.selection.y + this.selection.h) * tileSize * this.zoom);
        let x = l;
        let y = t
        let w = r - l;
        let h = b - t;
